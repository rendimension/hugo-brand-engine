...
print("🧠 Hugo Brand Engine v2.2 - Canva Specs Corrected con Fixes")
...
# Fonts: Inter Official Brand System - Prioriza carpeta fonts/
...
DESIGN_SYSTEM = {
    "carousel": {
        ...
        "header_gradient_color2": (0, 0, 0, 25),  # ~10% opacity
        
        # Footer (Bottom gradient bar)
        "footer_start": 1167,
        "footer_height": 183,
        "footer_gradient_color1": (0, 0, 0, 0),
        "footer_gradient_color2": (0, 0, 0, 77),  # ~30% opacity
        ...
        "brand_size": 48,  # Ajustado para matching Canva
        "brand_letter_spacing": 2,  # Pixeles entre letras
        "brand_y": 35,
...
def load_font(font_type: str, size: int):
    ...
    if os.path.isfile(path):
        ...
        try:
            ...
            print(f"❌ Error cargando {font_type}: {e} - fallback")
    else:
        print(f"❌ Fuente {font_type} NO encontrada en {path} - fallback")
...
def render_slide_v2(
    ...
    # === HEADER TEXT ===
    # Brand name con espaciado controlado
    ...
    # Subtitle con transparencia (si falla en Railway, comenta este bloque y usa draw.text sin layer)
    if subtitle:
        ...
        try:
            ...
            print(f"Error en transparencia subtitle: {e} - usando sin alpha")
            draw.text((margin_left, specs["subtitle_y"]), subtitle, font=subtitle_font, fill=(255, 255, 255))  # Fallback sin alpha
